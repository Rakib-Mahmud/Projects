...
for [source, headline, body] in news_list:
    ...
    do_truncate = rng.random() > 0.5
    if do_truncate:
        unigrams = unigrams[:int(len(unigrams)/2) +1]
...
create_table=True
if create_table:
    mycursor.execute("CREATE TABLE IF NOT EXISTS `result(truncate_random)` (row int,column_0 FLOAT)")
    i=0
    while (i < news_count):
        #sql="ALTER TABLE 'result(original)'  ADD %s FLOAT NULL  AFTER %s"
        sql="ALTER TABLE `result(truncate_random)` ADD column_%s FLOAT NULL AFTER column_%s"
        val=(i+1,i)
        mycursor.execute(sql,val)
        i+=1
        
mycursor.execute("DELETE FROM `result(truncate_random)`")
for i in range(news_count):
    for j in range(news_count):
        val=float(correlation_matrix[i][j])
        sql1="INSERT INTO `result(truncate_random)` (row, column_%s) VALUES (%s, %s)"
        val1=(j,i,val)
        sql2="UPDATE `result(truncate_random)` SET column_%s = %s WHERE row = %s"
        val2=(j,val,i)
        if j==0:
