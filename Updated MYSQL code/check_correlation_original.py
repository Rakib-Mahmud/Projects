...
create_table=True
if create_table:
    mycursor.execute("CREATE TABLE IF NOT EXISTS `result(original)` (row int,column_0 FLOAT)")
    i=0
    while (i < news_count):
        #sql="ALTER TABLE 'result(original)'  ADD %s FLOAT NULL  AFTER %s"
        sql="ALTER TABLE `result(original)` ADD column_%s FLOAT NULL AFTER column_%s"
        val=(i+1,i)
        mycursor.execute(sql,val)
        i+=1
        
mycursor.execute("DELETE FROM `result(original)`")
for i in range(news_count):
    for j in range(news_count):
        val=float(correlation_matrix[i][j])
        sql1="INSERT INTO `result(original)` (row, column_%s) VALUES (%s, %s)"
        val1=(j,i,val)
        sql2="UPDATE `result(original)` SET column_%s = %s WHERE row = %s"
        val2=(j,val,i)
        if j==0:
...
# =============================================================================
#         val.append(correlation_matrix[i][j])
#         col.append("column_"+str(j))
#         #col[j].format(j)
#     #col=str(col)
#     vl = json.dumps(val)
#     cl = json.dumps(col)
...
# =============================================================================
# =============================================================================
#     sql="INSERT INTO `result(original)`  VALUES %s"
